...
class myOauthDataAccess extends myOauthBase
{
  ...
  public function init($user_token_array, $oauth_encryption=OAUTH_SIG_METHOD_HMACSHA1, $oauth_type=OAUTH_AUTH_TYPE_URI)
  {
    ...
    {
      sfContext::getInstance()->getLogger()->info('{myOauthDataAccess} init: req:'.$user_token.' secret:'.$user_token_secret);
    }
  }
  ...
  public function updateStatus($text)
  {
    ...
    $this->oauth->fetch('http://twitter.com/statuses/update.json', $args);
    $json = json_decode($this->oauth->getLastResponse());
    return $json;
  }
}
